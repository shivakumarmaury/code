...
//=== New code added by Shareful on 2022-12-11. We can add any global function and action here ===//
$(function() {
    //=== New code added by Shareful to fix issue #AR-883, #AR-711, #AR-840 ===//
    ...
    //=== END of New code added by Shareful to fix issue #AR-883, #AR-711, #AR-840 ===//

    //=== New code added by Shareful on 2023-01-21 to fix issue #AR-1113, #AR-1128, #AR-1162 ===//
    $(document).mouseup(function(e) {
        // Hide left menu when clicking outside in mobile devices
        let leftMenu = $(".main-menu"); //=== Removed ".profile-main-menu.plr-none" by Shareful on 2023-05-05 to fix sidebar hide clicking outside ====//
    ...
    //=== END of New code added by Shareful on 2023-01-21 to fix issue #AR-1113, #AR-1128, #AR-1162 ===//

    //=== New code added by Shareful on 2023-05-01 to fix issue #AR-1533, #AR-1047, #AR-1039, #AR-1105 ===//
    $('form').on('reset', function(e) {
        setTimeout(function() {
            // executes after the form has been reset
            $(".key_notesaddition").trigger('keyup'); // Reset character count where displaying remaining characters count
        }, 1);
    });
    //=== END of New code added by Shareful on 2023-05-01 to fix issue #AR-1533, #AR-1047, #AR-1039, #AR-1105 ===//

    //=== apply bootstrap tooltip onfor dynamic element by Shareful on 2023-05-03 ===//
    $(document).tooltip({ selector: '[data-toggle="tooltip"]' });
});
/**
 * New code Added by Shareful on 2023-01-04 to fix issue #AR-1019, #AR-1100 globally
 * Overrite jquery native val() method to return value with trim()
 */
! function($) {
    ...
    $.fn.val = function(value) {
      ...
      //=== Updated code by Shareful on 2023-04-17 to fix javascript error for bootstrap dropdown field ===//
      if(typeof originalVal.call(this) !== 'string') {
        return originalVal.call(this, value); 
      } else {
        return originalVal.call(this).trim();
      }
      //=== Updated code ENDED here by Shareful on 2023-04-17 to fix javascript error for bootstrap dropdown field ===//
    };
}(jQuery);
/**
 * END of New code Added by Shareful on 2023-01-04 to fix issue #AR-1019, #AR-1100 globally
 */

//=== Added by Shareful on 2023-01-05 to fix issue #AR-1038, #1088 ===//
if(typeof mobileFormat != 'function'){
    window.mobileFormat = function(e){
...
function phoneNumbersOnly(input) {
...
//=== END of new code Added by Shareful on 2023-01-05 to fix issue #AR-1038, #1088 ===//

//=== This function is used for global display notification message (success or error or warning) based type added by Shareful for issue #AR-1306 ===//
function notification_msg(id, type, message) {
    if (type == "success") {
      if ($('#custom_success_message').length > 0) {
        $('#custom_success_message').remove();
      }
      //=== New success mesage design implemented by Shareful on 2023-04-08 ===//
      let html = `<div class="success_design" id="custom_success_message" data-width="" style="display: block; visibility: hidden">
        <div class="alert alert-success alert-dismissible d-flex align-items-center fade show">
          <i class="fa fa-check-circle"></i> ${message}
        </div>
      </div>`;
      $('body').append(html);
      // let boxHeight = $("#custom_success_message").height();
      let boxWidth = $("#custom_success_message").width();
      // $("#custom_success_message").css("width","0px").css("height",boxHeight).show().animate({width: boxWidth});
      $("#custom_success_message").css({"right": "-"+(boxWidth+25)+"px", "visibility": 'visible'}).animate({right: 0});

      setTimeout(function() {
        $("#custom_success_message").animate({right: "-"+(boxWidth+25)+"px"}); 
      }, 5000);
      //=== New success mesage design implementation has been ENDED here by Shareful on 2023-04-08 ===//
        ...
        //     <p class="text-left mb-15 alert alert-success"><i class="fa fa-check-circle"></i> ${message}</p>
      ...
      if ($('#custom_error_message_modal').length > 0) {
        $('#custom_error_message_modal').remove();
      }
      let modalTemplate = `<div class="modal fade" id="custom_error_message_modal" role="dialog">
        <div class="modal-dialog modal-xs" role="document">
        <div class="modal-content">
          <div class="modal-body">
          <h4 class="text-left">Error</h4>
          <p class="text-left mb-15 alert alert-danger"><i class="fa fa-exclamation-triangle"></i> ${message}</p>
            <div class="login-form">
              <div class="text-left">
                <button type="submit" onclick="$('#custom_error_message_modal').modal('hide');" class="btn btn-info btn-xs">Okay</button>
              </div>
              </div>
          </div>
        </div>
...
//=== Global notification code Added by Shareful end here ===//
